...
every device (stream-copy concat can produce irregular timestamps
that some phone hardware decoders render as a black screen).
"""
...
def fetch_stock_clip(query: str, out_path: str, api_key: str) -> bool:
    ...
    try:
        ...
        # Cap at 720px wide — smaller source footage means far less memory
        # spent decoding it, which matters a lot on a 512MB host.
...
def render_scene_to_file(text: str, index: int, tmpdir: str, api_key: str, voice: str) -> str:
    ...
    duration = audio_clip.duration  # video length must not exceed real audio length
    ...
    # Explicitly release everything before moving to the next scene —
    # this is the key change that prevents memory from climbing scene
    # over scene until the process gets killed.
    try:
        scene.close()
    except Exception:
        pass
...
def generate_video(script_text: str, output_path: str, api_key: str = "",
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Re-encode during the final join (rather than -c copy stream-copy).
        # Stream-copying separately-encoded scene files can produce a file
        # with slightly irregular internal timestamps — software players
        # and frame-extraction tools tolerate that fine, but many phone
        # hardware video decoders don't: they render black frames while
        # audio (on its own, more forgiving timeline) keeps playing
        # normally. Re-encoding here guarantees one clean, continuous
        # video stream that plays correctly everywhere.
        subprocess.run(
