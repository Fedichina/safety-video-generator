...
def generate_ai_clip(prompt: str, out_path: str, hf_token: str = "", max_wait: float = 280.0) -> bool:
    """
    Generate a short video clip from a text prompt using the free
    Wan-AI/Wan2.1 model hosted on Hugging Face Spaces (shared free GPU
    quota — "ZeroGPU").

    The real API (confirmed via /debug-ai-api) is asynchronous:
      1. /t2v_generation_async kicks off the job, returns a rough
         cost/wait-time estimate (not the video itself).
      2. /status_refresh is polled repeatedly on the SAME client/session
         until it returns a non-empty generated_video dict.

    This is experimental: free hosted AI video generation can take
    several minutes (shared GPU queue), and quality/resolution is well
    below paid tools — but it costs nothing.

    hf_token: optional free Hugging Face account token. Without one,
    the free daily quota is very small; with one, it's meaningfully
    higher. Create one free at huggingface.co/settings/tokens.
    """
    try:
        import time
        ...
        cost_timesecs, estimated_wait = client.predict(
            prompt,
            "720*1280",
            False,
            -1,
        ...
        try:
            initial_wait = float(estimated_wait)
        except (TypeError, ValueError):
            initial_wait = 60.0
        deadline = time.time() + min(max(initial_wait * 1.5, 60.0), max_wait)

        while time.time() < deadline:
            time.sleep(6)
            result = client.predict(api_name="/status_refresh")
            generated_video = result[0] if isinstance(result, (list, tuple)) else result
            if generated_video and isinstance(generated_video, dict) and generated_video.get("video"):
                src_path = generated_video["video"]
