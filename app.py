    ...
    def worker():
        try:
            ...
            generate_video(script_text, output_path, api_key=api_key, voice=voice,
                            hf_token=hf_token, progress_cb=progress)
...
@app.route("/debug-ai-api")
def debug_ai_api():
    """
    Diagnostic route: prints the real, current API schema of the
    Wan-AI/Wan2.1 Hugging Face Space so we can confirm the exact
    parameter names/order match what generate_ai_clip() expects.
    """
    try:
        ...
        return f"<pre>Error connecting to Wan-AI/Wan2.1: {e}</pre>", 500
